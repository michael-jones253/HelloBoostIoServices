...
namespace AsyncIo
{
    UnixStreamConnection::UnixStreamConnection(std::shared_ptr<TcpDomainConnection> domainConnection, bool isClientSide) :
	...
	/// </summary>
	UnixStreamConnection::~UnixStreamConnection()
	{
	...
	bool UnixStreamConnection::Expired() const
	{
	...
	int UnixStreamConnection::AsyncWrite(std::string&& msg, bool nullTerminate)
	{
	...
	/// <returns>The backlog of messages queued for writing.</returns>
	int UnixStreamConnection::AsyncWrite(std::vector<uint8_t>&& msg)
	{
		auto conn = _domainConnection.lock();
		if (!conn)
		{
	...
	/// <param name="buf">The buffer to receive the data.</param>
	/// <param name="len">The amount to be consumed/copied.</param>
	void UnixStreamConnection::ConsumeInto(uint8_t* buf, int len)
	{
		auto conn = _domainConnection.lock();
		if (!conn)
		{
	...
	/// <param name="len">The amount to be consumed/copied.</param>
	void UnixStreamConnection::ConsumeTo(std::vector<uint8_t>& dest, int len)
	{
		auto conn = _domainConnection.lock();
		if (!conn)
		{
	...
	const uint8_t* UnixStreamConnection::Data() const
	{
		auto conn = _domainConnection.lock();
		if (!conn)
		{
	...
	/// </summary>
	/// <returns>A pointer to available received data.</returns>
    size_t UnixStreamConnection::Size() const
	{
		auto conn = _domainConnection.lock();
		if (!conn)
		{
	...
	void UnixStreamConnection::Consume(int len)
	{
		auto conn = _domainConnection.lock();
		if (!conn)
		{
	...
	UnixEndPoint UnixStreamConnection::GetDomainEndPoint() const
	{
		auto conn = _domainConnection.lock();
		if (!conn)
		{
	...
	/// </summary>
	void UnixStreamConnection::Close()
	{
		auto conn = _domainConnection.lock();
		if (!conn)
		{
